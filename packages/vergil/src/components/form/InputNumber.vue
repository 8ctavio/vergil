...
const props = defineProps({
	...
	modelValue: {
		...
		default: (props: { value: number | string }) => Number(props.value)
	},
	...
	max: {
		type: Number,
		default: Infinity,
		validator: (max: number, { min }: { min?: number }) => min ? (max > min) : true
	},
	fraction: {
		type: [Boolean, Number, Array] as PropType<boolean | number | [number, number]>,
		default: true,
		validator(v: boolean | number | [number, number]) {
			if(Array.isArray(v)) {
				const [min,max] = v
...
function formatDisplayedString() {
	...
	;(elements.input as HTMLInputElement).value = props.separator ? separateThousands(displayedValue) : displayedValue
...
function handleBeforeInput(event: InputEvent) {
	const { data, inputType, target } = event as Omit<InputEvent, 'target'> & {
		target: Omit<HTMLInputElement, 'selectionStart' | 'selectionEnd'> & {
	...
	if (props.separator) {
		...
		eventData.newString = newValue as string
	}
	eventData.newNumber = Number(/\d/.test(newValue as string) ? newValue : 0)
	eventData.cursorPosition = eventData.newString.length - cursorOffset!
}
function handleInput(event: Event) {
	const { target } = event
	;(target as HTMLInputElement).value = eventData.newString
	;(target as HTMLInputElement).setSelectionRange(eventData.cursorPosition, eventData.cursorPosition)

	model.update(clamp(eventData.newNumber, range.min, range.max))
	;(elements.input as HTMLInputElement).value = eventData.newString
...
function handleChange(event: Event) {
	if (displayedNumber < range.min || displayedNumber > range.max) {
		...
		(elements.input as HTMLInputElement).value = '0' + (fraction.minDigits > 0 ? '.' + '0'.repeat(fraction.minDigits) : '')
	} else {
		...
		if(numStart > minus.length || numEnd) {
			(elements.input as HTMLInputElement).value = minus + leadingZero + value.slice(numStart, numEnd) + trailingZeros
		} else if(leadingZero || trailingZeros) {
			(elements.input as HTMLInputElement).value = leadingZero + value + trailingZeros
		}
	}
}
...
function countFractionalDigits(num: number | string, radixIDX?: number) {
	num = num.toString()
	radixIDX ??= num.indexOf('.')
	return radixIDX > -1 ? (num.length - radixIDX - 1) : 0
}
function sum(a: number, b: number, fractionalDigits: number) {
...
*/
function getDeleteSelection(inputType: string, target: HTMLInputElement) {

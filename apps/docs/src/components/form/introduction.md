...
<script setup>
import { InputText, CheckboxGroup } from 'vergil/components'
...
FFCs may be considered to have an associated *validation target*. This validation target is the FFC's model if it does not belong to a [model group](/composables/ModelGroup) or its model group ancestors do not have [group validators](/composables/ModelGroup#model-group-validation). Otherwise, the validation target is the model's eldest model group ancestor with a group validator.
...
FFCs that perform lazy or eager validation debouncing accept, respectively, `validation-delay` and `validation-cooldown` props to adjust the corresponding, underlying debounced function's [`minWait`](/utilities#debounce) parameter.
